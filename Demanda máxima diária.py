...
colunas_dias_meses_anos = [f'{d:02d} {m:02d} {ano}' for d in dias for m in meses for ano in anos]
df_dias_meses_anos = pd.DataFrame(index=df_dados_tecnicos.index, columns=colunas_dias_meses_anos)
print(df_dias_meses_anos)
########################################################################################################################
#df_datas_maximas = pd.DataFrame(index=df_atributos['Cód. do Trafo/Alimentador'], columns=colunas_dias_meses_anos)
df_dados_com_dias_meses_anos = pd.concat([df_dados_tecnicos, df_dias_meses_anos], axis=1)
df_dados_com_dias_meses_anos = df_dados_com_dias_meses_anos.fillna(0)
print("DF_DADOS_COM_DIAS_MESES_ANOS")
print(df_dados_com_dias_meses_anos)
#df_datas_maximas = pd.DataFrame(columns=colunas_dias_meses_anos)
#df_valores_maximo_P = pd.DataFrame(columns = ['Cód. do Trafo/Alimentador'] + colunas_dias_meses_anos)
            ...
            for dia in range(1,22):
                if dia in df_filtrado_mes["DIA"].values:
                    ...
                    for selecao in df_atributos['Cód. do Trafo/Alimentador']:
                        ...
                        df_base_filtrado = pd.DataFrame(df_filtrado_dia, columns=['DATA_HORA',descricao, descricao_saida, descricao_Q, descricao_saida_Q])
                        ...
                        if base_filtrada.size > 0:
                            valor_maximo_filtrado = np.nanmax(base_filtrada)
                        else:
                            # Defina um valor padrão, como zero, para valor_maximo_filtrado
                            valor_maximo_filtrado = 0
                        #valor_maximo_filtrado = np.nanmax(base_filtrada)
                        ...
                        print(potencia_aparente_maxima)
                        print(dia)
                        print(mes)
                        print(ano)
                        ...
                        if valor_maximo_P is not None:
                            ...
                            coluna_data = f'{dia:02d} {mes:02d} {ano}'
                            df_dados_com_dias_meses_anos.loc[linha, coluna_data] = valor_maximo_S
...
df_dados_com_dias_meses_anos['Potencia Instalada'] = pd.to_numeric(df_dados_com_dias_meses_anos['Potencia Instalada'], errors='coerce')
excluir_colunas= ['Descrição', 'Cód. de Ident', 'Cód. do Trafo/Alimentador', 'Potencia Instalada','Tensão Prim','Tensão Sec. (kV)']

df_dados_com_dias_meses_anos['Pot. Máxima'] = df_dados_com_dias_meses_anos.drop(excluir_colunas, axis=1).max(axis=1).round(2)
# Divide o máximo de cada linha pela coluna 'Potencia Instalada' se for maior que zero

#COLUNAS NÃO NUMERICAS
non_numeric_columns = df_dados_com_dias_meses_anos.select_dtypes(exclude=[np.number]).columns
print("Colunas não numéricas:", non_numeric_columns)
df_dados_com_dias_meses_anos['Pot. Máxima'] = df_dados_com_dias_meses_anos.drop(excluir_colunas, axis=1).max(axis=1).round(2)
df_dados_com_dias_meses_anos['Carregamento'] = (df_dados_com_dias_meses_anos['Pot. Máxima'] / df_dados_com_dias_meses_anos['Potencia Instalada'].where(df_dados_com_dias_meses_anos['Potencia Instalada'] > 0, other=1))*100
df_dados_com_dias_meses_anos['Carregamento'] =df_dados_com_dias_meses_anos['Carregamento'].round(2)
df_dados_com_dias_meses_anos.set_index('Cód. do Trafo/Alimentador')
#df_dados_com_dias_meses_anos["Ultrapassagem"] = sum(df_dados_com_dias_meses_anos['ultrapassagem'])
#print(df_dados_com_dias_meses_anos)
...
df_dados_com_dias_meses_anos['Tipo'] = df_dados_com_dias_meses_anos['Cód. do Trafo/Alimentador'].apply(define_tipo)
df_dados_com_dias_meses_anos.loc[df_dados_com_dias_meses_anos['Tipo'] == 'Alimentador', 'Cód. do Trafo/Alimentador'] = 'AL-' + df_dados_com_dias_meses_anos['Cód. do Trafo/Alimentador'].astype(str)

# Agora, o DataFrame df_dados_com_meses_anos possui uma nova coluna 'Tipo' com as informações desejadas
print(df_dados_com_dias_meses_anos)
########################################################################################################################
################################### EXPORTANDO O ARQUIVO ###############################################################
df_dados_com_dias_meses_anos = df_dados_com_dias_meses_anos.loc[:, ~df_dados_com_dias_meses_anos.columns.duplicated()]
...
dir = r"C:\Users\Engeselt\Documents\GitHub\ASPO\Valores_maximos_diários.xlsx"
df_dados_com_dias_meses_anos.to_excel(dir, sheet_name="Potência Ativa Máxima",engine='openpyxl', index=False)
