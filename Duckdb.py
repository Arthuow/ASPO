...
class AplicacaoProcessamento:
    def __init__(self):
        ...
        self.caminho = base_path / "Medicoes.duckdb"  
    ...
    def criar_banco_duckdb(self, caminho):
        if not os.path.exists(caminho):
            conn = duckdb.connect(caminho)
            conn.close()
            self.logger.info(f"Banco DuckDB criado em: {caminho}")
           
    def iniciar(self):
        self.logger.info("Iniciando processo de atualização da base de dados (DuckDB)")
        ...
        if not pasta_zip:  # Se nenhuma pasta foi selecionada
            self.logger.error("❌ Nenhuma pasta foi selecionada. Operação cancelada.")
            return
        ...
        try:
            ...
            if lista_dfs:
                ...
                # Converter Data_Hora para string no formato DuckDB
                ...
                self.logger.info("Criando banco DuckDB...")
                self.criar_banco_duckdb(self.caminho)
                
                # Enviar para DuckDB
                self.enviar_para_duckdb(df_final, 'DemandaDiaria', self.caminho)
            else:
    ...
    def enviar_para_duckdb(self, df, tabela, caminho_banco='Medicoes.duckdb'):
        try:
            self.logger.info("🔗 Conectando ao banco DuckDB...")
            ...
            tabela_existe = conn.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='{tabela}'").fetchone() is not None
            ...
            # Inserir dados usando o método mais eficiente do DuckDB
            total = len(df)
 
            if total == 0:
                self.logger.warning("⚠️ Nenhum dado válido para inserir no DuckDB. Verifique os arquivos de origem.")
                return
 
            # Registrar o DataFrame como uma tabela temporária
            conn.register('temp_df', df)
            
            # Inserir dados usando INSERT INTO ... SELECT
            conn.execute(f'''
                INSERT INTO {tabela} (Data_Hora, ponto_medicao, valor)
                SELECT Data_Hora, ponto_medicao, valor
                FROM temp_df
                ON CONFLICT (Data_Hora, ponto_medicao) 
            ...
            self.logger.error(f"❌ Erro ao enviar dados para o DuckDB: {e}")
